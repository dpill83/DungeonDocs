...
If a Reaction would trigger when a creature casts a spell—such as the Reaction taken to cast [Counterspell](../spells/counterspell.md)—it also triggers when you take this action to initiate a Circle spell.
...
When you cast a spell that requires at least one Material component with a specified cost that is consumed by the spell, you can reduce the minimum cost of one such Material component by 50 GP per each secondary caster contributing to the spell. For example, casting [Revivify](../spells/revivify.md) as a Circle spell with two secondary casters would reduce the Material component’s minimum cost to 200 GP.
