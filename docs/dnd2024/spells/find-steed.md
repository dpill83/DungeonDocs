...
You summon an otherworldly being that appears as a loyal steed in an unoccupied space of your choice within range. This creature uses the
Otherworldly Steed
stat block. If you already have a steed from this spell, the steed is replaced by the new one.
...
Hit Points:
5 + 10 per spell level (the steed has a number of Hit Dice [d10s] equal to the spell’s level)
Speed:
60 ft., Fly 60 ft. (requires level 4+ spell)
STR (Mod/Save)
DEX (Mod/Save)
CON (Mod/Save)
INT (Mod/Save)
