...
- **Components**: Use "V" (verbal), "S" (somatic), "M" (material). For material components, add parentheses with the exact description and cost when given, e.g. `M (a Holy Symbol worth 5+ GP)`. If you see "(C)" or "(C*)" in tables or older content: (C) = material component; (C*) = material component with cost or consumed—replace these with the full description when the source is known.
